...
<div class="max-w-3xl mx-auto space-y-6">
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-display font-bold text-slate-800">Editar Mão de Obra</h1>
        <p class="text-slate-500 mt-1">Atualize as informações do serviço de <strong>{{ $maoDeObra->nome_ou_tipo }}</strong>.</p>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
        <div class="absolute top-0 left-0 w-2 h-full bg-blue-500"></div>
        <div class="p-6 sm:p-8">
            <form action="{{ route('mao-de-obra.update', $maoDeObra->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nome_ou_tipo" id="nome_ou_tipo" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-blue-500 focus:ring-blue-500 rounded-xl shadow-sm transition-colors px-4 py-2.5" value="{{ $maoDeObra->nome_ou_tipo }}" required placeholder="Ex: Diarista / Tratorista">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="custo_diario_hora" id="custo_diario_hora" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-blue-500 focus:ring-blue-500 rounded-xl shadow-sm transition-colors pl-9 pr-4 py-2.5" value="{{ $maoDeObra->custo_diario_hora }}" placeholder="0.00">
                        </div>
                    </div>
                </div>
