...
@section('header_title', 'Mão de Obra')
                ...
                <tr class="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td class="px-6 py-4 text-right sticky right-0 bg-white group-hover:bg-gray-50/50 z-10 shadow-[-12px_0_15px_-3px_rgba(0,0,0,0.05)]">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('mao-de-obra.edit', $item->id) }}" class="p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors shadow-sm" title="Editar">
                            ...
                            <form action="{{ route('mao-de-obra.destroy', $item->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors shadow-sm" title="Excluir" onclick="return confirm('Tem certeza que deseja excluir este item?')">
                ...
                <tr>
                    <td colspan="@can('is-admin') 4 @else 3 @endcan" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center text-gray-500">
                            ...
                            <span class="text-sm font-medium">Nenhum item de mão de obra cadastrado.</span>
                        </div>
                    </td>
                </tr>
                @endforelse
