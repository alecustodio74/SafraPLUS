...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-display font-bold text-slate-800">Gerenciamento de Safras</h1>
            <p class="text-slate-500 mt-1">Acompanhe e gerencie as temporadas de plantio e colheita.</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('safras.create') }}" class="inline-flex items-center gap-2 text-sm font-bold text-white bg-primary-600 px-4 py-2 rounded-lg shadow-md shadow-primary-500/30 hover:bg-primary-700 hover:shadow-lg transition-all">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 text-sm font-semibold text-slate-600">
                        <th class="py-4 px-6">Cultura</th>
                        @can('is-admin')
                        <th class="py-4 px-6">Produtor</th>
                        @endcan
                        <th class="py-4 px-6 text-right">Área (ha)</th>
                        <th class="py-4 px-6">Localização</th>
                        <th class="py-4 px-6 whitespace-nowrap">Período</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        <td class="py-4 px-6">
                            <div class="font-medium text-slate-800 flex items-center gap-3 whitespace-nowrap">
                        ...
                        <td class="py-4 px-6 text-sm text-slate-600">
                            {{ $safra->produtor->nome ?? 'N/A' }}
                        </td>
                        @endcan
                        <td class="py-4 px-6 text-sm font-medium text-slate-700 text-right">
                            {{ number_format($safra->area_plantada, 2, ',', '.') }}
                        </td>
                        <td class="py-4 px-6 text-sm text-slate-600">
                            {{ $safra->localizacao ?: 'Não informada' }}
                        </td>
                        <td class="py-4 px-6 text-sm whitespace-nowrap">
                            <div class="flex flex-col text-xs">
                                ...
                                @if($safra->data_fim)
                                <span class="text-slate-500 flex items-center gap-1 mt-1">
                                    <svg class="w-3 h-3 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    {{ \Carbon\Carbon::parse($safra->data_fim)->format('d/m/Y') }}
                                </span>
                                @else
                                <span class="text-amber-600 font-medium flex items-center gap-1 mt-1 bg-amber-50 px-1.5 rounded-md inline-block max-w-max">
                        ...
                        <td class="py-4 px-6 text-right space-x-2 opacity-100 xl:opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                            <a href="{{ route('safras.edit', $safra->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-amber-600 hover:bg-amber-100 transition-colors" title="Editar">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </a>
                            <form action="{{ route('safras.destroy', $safra->id) }}" method="POST" class="inline-block" onsubmit="return confirm('ATENÇÃO: Tem certeza que deseja excluir esta safra e TODOS os seus lançamentos financeiros atrelados?');">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="@can('is-admin') 6 @else 5 @endcan" class="py-8 px-6 text-center text-slate-500">
                            <div class="flex flex-col items-center justify-center">
                                <svg class="w-12 h-12 text-slate-300 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                <p>Nenhuma safra cadastrada ainda.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="bg-slate-50 border-t border-slate-100 px-6 py-3 text-sm text-slate-500">
            Total de safras: <span class="font-bold text-slate-700">{{ $safras->count() }}</span>
        </div>
        @endif
    </div>
</div>
