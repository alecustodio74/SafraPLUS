...
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-display font-bold text-slate-800">Editar Safra</h1>
        <p class="text-slate-500 mt-1">Atualize as informações do plantio e colheita.</p>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
        <div class="absolute top-0 left-0 w-2 h-full bg-orange-500"></div>
        <div class="p-6 sm:p-8">
            <form action="{{ route('safras.update', $safra->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                @can('is-admin')
                    <div class="mb-6">
                        ...
                        <select name="produtor_id" id="produtor_id" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-orange-500 focus:ring-orange-500 rounded-xl shadow-sm transition-colors px-4 py-2.5 appearance-none" required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    @php
                        $isCulturaPredefinida = in_array($safra->cultura, $culturas);
                        $valorSelectInicial = $isCulturaPredefinida ? $safra->cultura : 'outros';
                        $valorOutroInicial = $isCulturaPredefinida ? '' : $safra->cultura;
                    @endphp
                    <div x-data="{ 
                            culturaSelecionada: '{{ $valorSelectInicial }}', 
                            outraCultura: {{ $isCulturaPredefinida ? 'false' : 'true' }},
                            init() {
                        ...
                        <div 
                            ...
                            style="display: {{ $isCulturaPredefinida ? 'none' : 'block' }};"
                        >
                            <input 
                                x-ref="inputCultura"
                                type="text" 
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" step="0.01" name="area_plantada" id="area_plantada" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-orange-500 focus:ring-orange-500 rounded-xl shadow-sm transition-colors pr-12 pl-4 py-2.5" value="{{ $safra->area_plantada }}" placeholder="Ex: 200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="data_inicio" id="data_inicio" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-orange-500 focus:ring-orange-500 rounded-xl shadow-sm transition-colors px-4 py-2.5" value="{{ $safra->data_inicio }}" required>
                    </div>
                    <div>
                        <label for="data_fim" class="block text-sm font-medium text-slate-700 mb-1">Data de Fim (Opcional)</label>
                        <input type="date" name="data_fim" id="data_fim" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-orange-500 focus:ring-orange-500 rounded-xl shadow-sm transition-colors px-4 py-2.5 text-slate-500" value="{{ $safra->data_fim }}">
                ...
                <div>
                    ...
                    <input type="text" name="localizacao" id="localizacao" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-orange-500 focus:ring-orange-500 rounded-xl shadow-sm transition-colors px-4 py-2.5" value="{{ $safra->localizacao }}" placeholder="Ex: Sede Talhão 4, ou Fazenda Esperança">
                </div>
