...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-display font-bold text-slate-800">Cadastro de Insumos</h1>
            <p class="text-slate-500 mt-1">Gerencie os insumos da sua propriedade agropecuária.</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('insumos.create') }}" class="inline-flex items-center gap-2 text-sm font-bold text-white bg-primary-600 px-4 py-2 rounded-lg shadow-md shadow-primary-500/30 hover:bg-primary-700 hover:shadow-lg transition-all">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 text-sm font-semibold text-slate-600">
                        <th class="py-4 px-6">Nome do Insumo</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        ...
                        @can('is-admin')
                        <td class="py-4 px-6 text-sm text-slate-600">
                            {{ $insumo->produtor->nome ?? 'N/A' }}
                        </td>
                        @endcan
                        <td class="py-4 px-6 text-right space-x-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('insumos.edit', $insumo->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-amber-600 hover:bg-amber-100 transition-colors" title="Editar">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </a>
                            <form action="{{ route('insumos.destroy', $insumo->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Tem certeza que deseja excluir este insumo?');">
                                @csrf
                                @method('DELETE')
        ...
        <div class="bg-slate-50 border-t border-slate-100 px-6 py-3 text-sm text-slate-500">
            Total de insumos cadastrados: <span class="font-medium text-slate-700">{{ $insumos->count() }}</span>
        </div>
        @endif
    </div>
</div>
