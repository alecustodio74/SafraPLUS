...
@section('header_title', 'Movimentações de Estoque')
    ...
    <div class="overflow-x-auto pl-1.5">
        ...
        <table class="w-full text-left border-collapse hidden md:table">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100 text-xs uppercase tracking-wider text-gray-500 font-bold">
                    ...
                    <th class="px-6 py-4">Safra (Saída)</th>
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td class="px-6 py-4 font-semibold text-gray-900">
                        {{ $movimentacao->insumo->nome ?? 'N/A' }}
                    </td>
                    <td class="px-6 py-4 text-gray-600 font-medium whitespace-nowrap">
                        {{ $movimentacao->quantidade }}
                    </td>
                    <td class="px-6 py-4 text-gray-600">
                        {{ $movimentacao->safra->cultura ?? '—' }}
                    </td>
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        R$ {{ number_format($movimentacao->quantidade * $movimentacao->valor_unitario, 2, ',', '.') }}
                    </td>
                    @can('is-admin')
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right sticky right-0 bg-white group-hover:bg-gray-50/50 z-10 shadow-[-12px_0_15px_-3px_rgba(0,0,0,0.05)]">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('movimentacoes-estoque.edit', $movimentacao->id) }}" class="p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors shadow-sm" title="Editar">
                            ...
                            <form action="{{ route('movimentacoes-estoque.destroy', $movimentacao->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors shadow-sm" title="Excluir" onclick="return confirm('Tem certeza que deseja excluir esta movimentação?')">
                ...
                <tr>
                    <td colspan="@can('is-admin') 8 @else 7 @endcan" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center text-gray-500">
                            ...
                            <span class="text-sm font-medium">Nenhuma movimentação cadastrada.</span>
                        </div>
                    </td>
                </tr>
                @endforelse
        ...
        <div class="md:hidden flex flex-col p-4 gap-4 bg-gray-50/50">
            ...
            <div class="bg-white rounded-xl p-4 border border-gray-100 flex flex-col shadow-sm relative">
                ...
                <div class="mb-2">
                    <span class="text-xs text-gray-500 font-medium">{{ \Carbon\Carbon::parse($movimentacao->data_movimentacao)->format('d/m/Y') }}</span>
                </div>
                ...
                <div class="mb-3 pr-16">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Valor Total: <span class="font-bold text-gray-900">R$ {{ number_format($movimentacao->quantidade * $movimentacao->valor_unitario, 2, ',', '.') }}</span></p>
