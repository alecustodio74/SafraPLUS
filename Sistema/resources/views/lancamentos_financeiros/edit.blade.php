...
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-display font-bold text-slate-800">Editar Lançamento Financeiro</h1>
        <p class="text-slate-500 mt-1">Altere os dados deste lançamento.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden" x-data="{ tipoLancamento: '{{ old('tipo_receita_custo', $lancamento->tipo_receita_custo) }}' }">
        <div class="p-6 sm:p-8">
            <form action="{{ route('lancamentos-financeiros.update', $lancamento->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Safra ID -->
                    <div class="md:col-span-2">
                        <label for="safra_id" class="block text-sm font-medium text-slate-700 mb-1">Associar à Safra <span class="text-red-500">*</span></label>
                        <select name="safra_id" id="safra_id" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2.5 appearance-none" required>
                            ...
                            @foreach($safras as $safra)
                                <option value="{{ $safra->id }}" {{ (old('safra_id') ?? $lancamento->safra_id) == $safra->id ? 'selected' : '' }}>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="categoria_id" id="categoria_id" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2.5 appearance-none" required>
                            ...
                            @foreach($categorias as $categoria)
                                <option value="{{ $categoria->id }}" {{ (old('categoria_id') ?? $lancamento->categoria_id) == $categoria->id ? 'selected' : '' }}>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="descricao" id="descricao" value="{{ old('descricao', $lancamento->descricao) }}" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2" required placeholder="Ex: Venda de Soja, Compra de Sementes...">
                        @error('descricao')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="valor_total" id="valor_total" value="{{ old('valor_total', $lancamento->valor_total) }}" class="w-full pl-10 border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2" required placeholder="0,00">
                        </div>
                        @error('valor_total')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="data_lancamento" id="data_lancamento" value="{{ old('data_lancamento', $lancamento->data_lancamento) }}" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2" required>
