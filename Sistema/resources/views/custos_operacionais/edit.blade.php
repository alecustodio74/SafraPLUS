...
@section('header_title', 'Editar Custo Operacional')

@section('content')
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <p class="text-sm text-gray-500 mt-1">Atualize as informações desta despesa.</p>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden max-w-2xl">
    ...
    <div class="p-8 pl-10">
        <form action="{{ route('custos-operacionais.update', $custo->id) }}" method="POST">
            ...
            @php
                $categoriaInicial = $custo->categoria ?? 'geral';
                if (!$custo->categoria) {
                    if ($custo->mao_de_obra_id) {
                        $categoriaInicial = 'mao_de_obra';
                    } elseif ($custo->maquinario_id) {
                        $categoriaInicial = 'maquinario';
                    } elseif (str_starts_with(strtolower($custo->descricao), 'combust')) {
                        $categoriaInicial = 'combustivel';
                    } elseif (str_starts_with(strtolower($custo->descricao), 'manuten')) {
                        $categoriaInicial = 'manutencao';
                    }
                }
            @endphp
            ...
            <div class="mb-5">
                ...
                <select id="categoria_custo" name="categoria" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-rose-500 focus:border-rose-500 px-4 py-3 transition-colors outline-none">
                    <option value="geral" {{ $categoriaInicial == 'geral' ? 'selected' : '' }}>Geral / Outros</option>
                    <option value="combustivel" {{ $categoriaInicial == 'combustivel' ? 'selected' : '' }}>Combustível</option>
                    <option value="manutencao" {{ $categoriaInicial == 'manutencao' ? 'selected' : '' }}>Manutenção</option>
                    <option value="mao_de_obra" {{ $categoriaInicial == 'mao_de_obra' ? 'selected' : '' }}>Mão de Obra</option>
                    <option value="maquinario" {{ $categoriaInicial == 'maquinario' ? 'selected' : '' }}>Maquinário</option>
                </select>
            </div>
            ...
            <div class="mb-5">
                ...
                <input type="text" id="descricao_geral" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-rose-500 focus:border-rose-500 px-4 py-3 transition-colors outline-none" value="{{ in_array($categoriaInicial, ['geral', 'combustivel', 'manutencao']) ? $custo->descricao : '' }}" placeholder="Ex: Combustível, Manutenção">
                ...
                <select id="mao_de_obra_id_select" class="hidden w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-rose-500 focus:border-rose-500 px-4 py-3 transition-colors outline-none">
                    ...
                    @foreach($maoDeObras as $item)
                        <option value="{{ $item->id }}" data-custo="{{ $item->custo_diario_hora }}" {{ $custo->mao_de_obra_id == $item->id ? 'selected' : '' }}>{{ $item->nome_ou_tipo }}</option>
                    @endforeach
                </select>
                ...
                <select id="maquinario_id_select" class="hidden w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-rose-500 focus:border-rose-500 px-4 py-3 transition-colors outline-none">
                    ...
                    @foreach($maquinarios as $maquinario)
                        <option value="{{ $maquinario->id }}" data-custo="{{ $maquinario->custo_hora }}" {{ $custo->maquinario_id == $maquinario->id ? 'selected' : '' }}>{{ $maquinario->nome }}</option>
                    @endforeach
                </select>
                
                <input type="hidden" name="descricao" id="descricao" value="{{ $custo->descricao }}">
                <input type="hidden" name="mao_de_obra_id" id="mao_de_obra_id" value="{{ $custo->mao_de_obra_id }}">
                <input type="hidden" name="maquinario_id" id="maquinario_id" value="{{ $custo->maquinario_id }}">
            </div>

            <div class="flex flex-col md:flex-row gap-5 mb-5">
                <div class="flex-1">
                    <label for="data" class="block text-sm font-medium text-gray-700 mb-2">Data</label>
                    <input type="date" name="data" id="data" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-rose-500 focus:border-rose-500 px-4 py-3 transition-colors outline-none" value="{{ $custo->data }}" required>
                </div>
                <div class="flex-1">
                    <label for="quantidade" id="label_quantidade" class="block text-sm font-medium text-gray-700 mb-2">Quantidade (QTD)</label>
                    <input type="number" step="0.01" name="quantidade" id="quantidade" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-rose-500 focus:border-rose-500 px-4 py-3 transition-colors outline-none" value="{{ $custo->quantidade }}" placeholder="Ex: 2.5" oninput="calcularValorTotal()">
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-5 mb-5">
                <div class="flex-1">
                    ...
                    <input type="number" step="0.01" name="preco_unitario" id="preco_unitario" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-rose-500 focus:border-rose-500 px-4 py-3 transition-colors outline-none" value="{{ $custo->preco_unitario }}" placeholder="Ex: 150,00" oninput="calcularValorTotal()">
                </div>
                <div class="flex-1">
                    <label for="valor" class="block text-sm font-medium text-gray-700 mb-2">Valor Total</label>
                    <input type="number" step="0.01" name="valor" id="valor" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl focus:ring-rose-500 focus:border-rose-500 px-4 py-3 transition-colors outline-none" value="{{ $custo->valor }}" required placeholder="Ex: 500,00">
                </div>
            </div>
