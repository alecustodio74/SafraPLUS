...
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-display font-bold text-slate-800">Novo Custo Operacional</h1>
        <p class="text-slate-500 mt-1">Registre um novo custo associado a uma safra.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-6 sm:p-8">
            <form action="{{ route('custos-operacionais.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="descricao" id="descricao" value="{{ old('descricao') }}" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2" required placeholder="Ex: Combustível, Manutenção...">
                        @error('descricao')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="data" id="data" value="{{ old('data', date('Y-m-d')) }}" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2" required>
                        @error('data')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="valor" id="valor" value="{{ old('valor') }}" class="w-full pl-10 border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2" required placeholder="0,00">
                        </div>
                        @error('valor')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        ...
                        <div>
                            ...
                            <select name="maquinario_id" id="maquinario_id" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2.5 appearance-none">
                                ...
                                @foreach($maquinarios as $maquinario)
                                    <option value="{{ $maquinario->id }}" {{ old('maquinario_id') == $maquinario->id ? 'selected' : '' }}>{{ $maquinario->nome }}</option>
                                @endforeach
                            </select>
                            @error('maquinario_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select name="mao_de_obra_id" id="mao_de_obra_id" class="w-full border-slate-300 bg-slate-50 text-slate-900 focus:bg-white focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm transition-colors px-4 py-2.5 appearance-none">
                                ...
                                @foreach($maoDeObras as $item)
                                    <option value="{{ $item->id }}" {{ old('mao_de_obra_id') == $item->id ? 'selected' : '' }}>{{ $item->nome_ou_tipo }}</option>
                                @endforeach
                            </select>
